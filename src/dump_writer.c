...
// `type`.
static bool
ihex_record (File *f, uint8_t count, uint16_t addr, uint8_t type,
             const uint8_t *data)
{
    char    line[64];
    int     pos = snprintf(line, sizeof(line), ":%02X%04X%02X", (unsigned)count,
...
static void
hex_flush_record (DumpWriter *w, uint32_t start_addr, const uint8_t *data,
                  uint8_t count)
{
    ...
    w->ok =
        ihex_record(w->hex, count, (uint16_t)(start_addr & 0xFFFF), 0x00, data);
}

bool
dump_writer_open (DumpWriter *w, Storage *s, const char *bin_path,
                  const char *hex_path, uint32_t base)
{
    ...
    if (bin_path)
    {
        ...
        if (!storage_file_open(w->bin, bin_path, FSAM_WRITE,
                               FSOM_CREATE_ALWAYS))
        {
            w->ok = false;
        }
    ...
    if (hex_path)
    {
        ...
        if (!storage_file_open(w->hex, hex_path, FSAM_WRITE,
                               FSOM_CREATE_ALWAYS))
        {
            w->ok = false;
        }
...
dump_writer_close (DumpWriter *w)
{
    if (w->hex)
    {
        if (w->ok && w->rec_len > 0)
        {
            hex_flush_record(w, w->base + (w->pos - w->rec_len), w->rec,
                             w->rec_len);
            w->rec_len = 0;
        }
        if (w->ok)
...
bool
dump_write_ihex (Storage *s, const char *path, uint32_t base,
